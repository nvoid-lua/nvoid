local function update()
   ...
   local utils = require "ui"
   local git = require 'updater.utils.git'
   local misc = require 'updater.utils.misc'
   local prompts = require 'updater.utils.prompts'
   ...
   if continue == nil and skip_confirmation == nil then
      ...
      echo(misc.list_text_replace(prompts.reset_remote_head_success, "<UPDATE_BRANCH>",
         git.update_branch))
   ...
   local function update_exit(_, code)
      ...
      if code == 0 then
         ...
         local applied_commit_list = git.get_commit_list_by_hash_range(git.current_sha,
            git.get_local_head())
