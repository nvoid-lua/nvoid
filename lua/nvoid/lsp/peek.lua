...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range["end"].line + 1 + (opts.context or nvoid.lsp.peek.max_height),
      range.start.line + (opts.max_height or nvoid.lsp.peek.max_height)
    ),
    false
  )
  ...
  opts = vim.lsp.util.make_floating_popup_options(
    if_nil(width, nvoid.lsp.peek.max_width),
    if_nil(height, nvoid.lsp.peek.max_height),
    opts
  )
...
local function preview_location_callback(result)
  ...
  local opts = {
    border = "rounded",
    context = nvoid.lsp.peek.context,
  }
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    vim.api.nvim_buf_set_keymap(
      ...
      "n",
      "<CR>",
      ":lua require('nvoid.lsp.peek').open_file()<CR>",
