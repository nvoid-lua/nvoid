...
local nvoid_runtime = get_runtime_dir()
local nvoid_config = get_config_dir()
...
local function mod_path(path)
  ...
  path = path:gsub(nvoid_runtime .. "/", "<NVOID_RUNTIME>/")
  path = path:gsub(nvoid_config .. "/", "<NVOID_CONFIG>/")
  return path
end
