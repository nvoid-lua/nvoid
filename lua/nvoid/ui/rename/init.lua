...
local function rename(popup_opts, opts)
  ...
  local user_border = _G.Nvoid_user_opts.rename.border
  local width = 25
  ...
  popup_opts = utils.merge({
    ...
    border = {
      ...
      style = user_border.style or _G.Nvoid_user_opts.border_style,
      text = {
  ...
  opts = utils.merge({
    prompt = Text(_G.Nvoid_user_opts.rename.prompt, _G.Nvoid_user_opts.rename.prompt_hl),
